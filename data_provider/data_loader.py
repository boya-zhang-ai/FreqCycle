...
class Dataset_Pred(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, inverse=False, timeenc=0, freq='h', cycle=None,stride=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        week_day=df_stamp['weekday'].iloc[self.pred_len]
        print(week_day*96//self.stride[0])
        self.cycle_index=np.array([week_day*96//self.stride[0],0])
...
class Dataset_CustomM(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', cycle=None,stride=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        self.cycle_index = np.zeros((len(self.cycle), seq_length))  # 形状 (Cycle_num, seq_length)

        # add cycle
        for i in range(len(self.cycle)):
            self.cycle_index[i]=((np.arange(len(data)) // self.stride[i]) % (self.cycle[i]//self.stride[i]))[border1:border2]
        print(self.cycle_index.shape)
